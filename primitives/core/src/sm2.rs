...
#[cfg(feature = "full_crypto")]
use sm2::signature::{Pubkey, Seckey};
use sp_runtime_interface::pass_by::{Codec, PassBy, PassByInner};
#[cfg(feature = "std")]
...
/// A signature
#[derive(Encode, Decode, PassByInner, MaxEncodedLen, TypeInfo)]
...
impl TraitPair for Pair {
	...
	fn from_seed_slice(seed_slice: &[u8]) -> Result<Pair, SecretStringError> {
		let secret =
			sm2::utils::parse_sk(seed_slice).map_err(|_| SecretStringError::InvalidSeedLength)?;
	...
	fn verify_weak<P: AsRef<[u8]>, M: AsRef<[u8]>>(sig: &[u8], message: M, pubkey: P) -> bool {
		...
		Self::verify(
			&Signature::from_slice(sig.as_ref()),
			message,
			&Self::Public::from_slice(pubkey.as_ref()).unwrap(),
		)
	}
